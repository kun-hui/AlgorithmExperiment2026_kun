...
typedef struct { int id; int w; double v; double ratio; } Item;
typedef struct { double bestValue; long long bestWeight; unsigned char *chosen; int n; } KnapResult;

static unsigned long long cmp_count = 0ULL;
static FILE *subproblem_file = NULL;
static int record_subproblem = 0;
...
void copy_array(int *dst, int *src, int n) { memcpy(dst, src, sizeof(int) * n); }
...
void merge_sort_rec(int *a, int l, int r, int *tmp, int depth) {
    int n = r - l + 1;
    if (record_subproblem && subproblem_file) fprintf(subproblem_file, "merge,%d,%d\n", depth, n);
    if (l >= r) return;
    int m = l + (r - l) / 2;
...
void quick_sort_rec(int *a, int l, int r, int depth) {
    int n = r - l + 1;
    if (record_subproblem && subproblem_file && n > 0) fprintf(subproblem_file, "quick,%d,%d\n", depth, n);
    if (l >= r) return;
    ...
    for (int j = l; j < r; ++j) {
        cmp_count++;
        if (a[j] <= pivot) { int x = a[i]; a[i] = a[j]; a[j] = x; i++; }
    }
    t = a[i]; a[i] = a[r]; a[r] = t;
...
void quick_sort(int *a, int n) { cmp_count = 0ULL; quick_sort_rec(a, 0, n - 1, 0); }
...
KnapResult make_result(int n) {
    KnapResult r; r.bestValue = 0.0; r.bestWeight = 0; r.n = n;
    r.chosen = (unsigned char*)calloc(n, 1);
    if (!r.chosen) { fprintf(stderr, "calloc failed\n"); exit(1); }
    return r;
}

void free_result(KnapResult *r) { free(r->chosen); r->chosen = NULL; }
...
KnapResult dp_knap(Item *items, int n, int C) {
    ...
    if (!dp || !take) { fprintf(stderr, "DP memory insufficient for n=%d C=%d\n", n, C); exit(2); }
    ...
    int c = C;
    for (int i = n - 1; i >= 0; --i) {
        if (take[i*(C+1)+c]) { res.chosen[i] = 1; res.bestWeight += items[i].w; c -= items[i].w; }
    }
...
KnapResult brute_knap(Item *items, int n, int C) {
    ...
    for (unsigned long long mask = 0; mask < total; ++mask) {
        ...
        if (w <= C && v > res.bestValue) {
            ...
            memset(res.chosen, 0, n); for (int i = 0; i < n; ++i) if (mask & (1ULL << i)) res.chosen[i] = 1;
        }
    }
    return res;
}

static Item *bt_items; static int bt_n, bt_C; static double bt_best; static int bt_weight; static unsigned char *bt_x, *bt_bestx;
...
void bt_rec(int idx, int weight, double value) {
    ...
    if (weight + bt_items[idx].w <= bt_C) {
        bt_x[idx] = 1; bt_rec(idx+1, weight+bt_items[idx].w, value+bt_items[idx].v); bt_x[idx] = 0;
    }
    if (bound_value(idx+1, weight, value) > bt_best) bt_rec(idx+1, weight, value);
}

KnapResult backtrack_knap(Item *items, int n, int C) {
    if (n > 2000) { fprintf(stderr, "Backtracking is exponential; this demo caps n<=2000.\n"); exit(4); }
...
void print_knap_result(const char *name, KnapResult *r, Item *items) {
    printf("%s total_value=%.2f total_weight=%lld selected_ids=", name, r->bestValue, r->bestWeight);
    int printed = 0;
    for (int i = 0; i < r->n; ++i) if (r->chosen[i]) { if (printed < MAX_ITEMS_TO_PRINT) printf("%d ", i+1); printed++; }
...
int main(int argc, char **argv) {
    ...
    if (strcmp(argv[1], "sort") == 0) {
        ...
        if (argc >= 5) { subproblem_file = fopen(argv[4], "w"); record_subproblem = 1; fprintf(subproblem_file, "algorithm,depth,size\n"); }
        ...
        long long ed=now_ms(); print_knap_result(alg,&r,items); printf("time_ms=%lld\n", ed-st);
